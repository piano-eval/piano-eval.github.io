...
PageTemplateRenderer.prototype.renderNavigation = function(_parentId) {
  ...
  if (this.pageManager.getPageIndex() < (this.pageManager.getNumPages() - 1) || ![undefined, false].includes(this.pageManager.pages[this.pageManager.getPageIndex()].pageConfig.send)) {
    ...
    Mousetrap.bind('enter', function() {if (!buttonNext[0].hasAttribute('disabled')) {buttonNext.click();}});
