...
function FilePlayer(_audioContext, _bufferSize, _stimuli, _errorHandler, _language, _localizer, _hideProgressBar) {
  ...
  this.genericAudioControl.addEventListener((function (_event) {
    ...
    } else if (_event.name == 'ended') {
      $("#playPause"+_event.index).text(this.localizer.getFragment(this.language, 'playButton'));
    }
  }).bind(this));
...
FilePlayer.prototype.renderElement = function(_parent, _i) {
  var buttonPlayPause = $('<button data-role="button" id="playPause'+_i+'" name="'+_i+'"  class="audioControlElement">' + this.localizer.getFragment(this.language, 'playButton')  + '</button>');
  buttonPlayPause.bind( "click", (function(event, ui) {
    var eventName = parseInt(event.target.name);
    var label = $("#playPause"+eventName).text();
    if (label == this.localizer.getFragment(this.language, 'playButton')) {
      this.genericAudioControl.play(eventName);
      $("#playPause"+eventName).text(this.localizer.getFragment(this.language, 'pauseButton'));
      for (var i = 0; i < this.stimuli.length; ++i) {
        if (i !== eventName) {
          $("#playPause"+i).text(this.localizer.getFragment(this.language, 'playButton'));
        }
      }
    } else if (label == this.localizer.getFragment(this.language, 'pauseButton')) {
      this.genericAudioControl.pause();
      $("#playPause"+eventName).text(this.localizer.getFragment(this.language, 'playButton'));
    }
  }).bind(this));
